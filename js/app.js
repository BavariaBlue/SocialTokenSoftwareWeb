...
App = {
    ...
    $.getJSON('Social.json', function (data) {
      ...
      web3.eth.getAccounts(function (error, accounts) {
        ...
        App.contracts.Social.deployed().then(function (instance) {
          ...
          document.querySelector("spenderAddress").textContent = web3.toChecksumAddress(account);
          ...
          var itemTemplate10 = itemTemplate.clone();
          itemTemplate10.find('.panel-title').text("Weltenburger Urtyp");
          itemTemplate10.find('.itemPrice').text("899 ST");
          itemTemplate10.find('.input-number').attr('name', "quant[10]");
          itemTemplate10.find('.btn-number').attr('data-field', "quant[10]");
          itemTemplate10.find('.input-number').attr('data-name', "Weltenburger Urtyp");
          itemTemplate10.find('.input-number').attr('data-price', 899);
          itemTemplate10.find('.input-number').attr('data-id', 10);
          itemTemplate10.find('img').attr('src', "images/items/weltenburgerUrtyp.png");
          itemTemplate10.removeClass('hidden');

          var itemTemplate11 = itemTemplate.clone();
          itemTemplate11.find('.panel-title').text("Marlboro Red");
          itemTemplate11.find('.itemPrice').text("6699 ST");
          itemTemplate11.find('.input-number').attr('name', "quant[11]");
          itemTemplate11.find('.btn-number').attr('data-field', "quant[11]");
          itemTemplate11.find('.input-number').attr('data-name', "Marlboro Red Cigarettes");
          itemTemplate11.find('.input-number').attr('data-price', 6699);
          itemTemplate11.find('.input-number').attr('data-id', 11);
          itemTemplate11.find('img').attr('src', "images/items/marlboroRed.png");
          itemTemplate11.removeClass('hidden');

          var itemTemplate12 = itemTemplate.clone();
          itemTemplate12.find('.panel-title').text("Mon Chéri Chocolates");
          itemTemplate12.find('.itemPrice').text("1399 ST");
          itemTemplate12.find('.input-number').attr('name', "quant[12]");
          itemTemplate12.find('.btn-number').attr('data-field', "quant[12]");
          itemTemplate12.find('.input-number').attr('data-name', "Mon Chéri Chocolates");
          itemTemplate12.find('.input-number').attr('data-price', 1399);
          itemTemplate12.find('.input-number').attr('data-id', 12);
          itemTemplate12.find('img').attr('src', "images/items/monCheri.png");
          itemTemplate12.removeClass('hidden');

          itemRow.append(itemTemplate1.html() + itemTemplate2.html() + itemTemplate3.html() + itemTemplate4.html() + itemTemplate5.html() + itemTemplate6.html() + itemTemplate7.html() + itemTemplate8.html() + itemTemplate9.html() + itemTemplate10.html() + itemTemplate11.html() + itemTemplate12.html());
        })
      });
      if (!bindFunctions) {
        bindFunctions = true;
  ...
  changeInput: function () {
    ...
    shoppingCart[itemId - 1] = [itemName, itemPrice * valueFinal, valueFinal, itemId];
    App.changeBasket();
  },
  ...
  changeBasket: function () {
    ...
    var iterator = 0;
    document.querySelector("forbiddenProducts").textContent = "";
    $("div.forbiddenProductsInBasket").hide();
    ...
    shoppingCart.forEach(position => {
      if (position[2] != 0) {
        ...
        var productID = position[3];
        App.contracts.Social.deployed().then(async function (instance) {
          socialInstance = instance;
          if (await socialInstance.getBlacklistItem.call('0x96c65A2b5Ef50bc13f227A1f3c285e5045a4664B', productID)) {
            $("div.forbiddenProductsInBasket").show();
            $('.btn-buyItems').attr('disabled', true);
            if (iterator > 0) {
              document.querySelector("forbiddenProducts").textContent += ", ";
            }
            document.querySelector("forbiddenProducts").textContent += position[0];
            ++iterator;
          }
        })
      } else {
  ...
  spendToken: function (event) {
    ...
    // Generate a cleanShopping ID array without names, single price, and empty fields
    // == set parameter basketItems: send all selected itemIds, so that SC can disambiguate a legal and illegal purchase
    var cleanShoppingCart = [];
    for (var i = 0; i < shoppingCart.length; i++) {
      if (shoppingCart[i][2] != 0) {
        cleanShoppingCart.push(parseInt(shoppingCart[i][3]));
      }
    }
    web3.eth.getAccounts(function (error, accounts) {
      if (error) {
      ...
      App.contracts.Social.deployed().then(function (instance) {
        socialInstance = instance;
        return socialInstance.spendToken(sum, '0x96c65A2b5Ef50bc13f227A1f3c285e5045a4664B', cleanShoppingCart, {
          from: account
        });
